...
func TestSwipe_GetCountSwipe(t *testing.T) {
	query := `SELECT COUNT(*) FROM swipes WHERE swipe_user_id = $1 AND date(created_at) = $2`
	type args struct {
		ctx context.Context
	...
	tests := []struct {
	...
	}{
		{
			...
			want: 5, // Assume 5 swipes for testing
			setup: func(mock sqlmock.Sqlmock, a args) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call the function
			got, err := s.GetCountSwipe(tt.args.ctx, tt.args.req)
